...
distillation_prompt_template = """# Task Description
...
class Embedding_Model(nn.Module):
    ...
    def hac_cluster(self, cur_data):
        ...
        final_chosen_decision = []
        final_rejected_decision = []
        for item in ward2gptquerys:
            ...
            top1_candidate = sorted_candidates[0][0].strip()
            tail1_candidate = sorted_candidates[-1][0].strip()

            final_chosen_decision.append(top1_candidate)
            final_rejected_decision.append(tail1_candidate)

        cur_data["chosen"] = final_chosen_decision
        cur_data["rejected"] = final_rejected_decision
    ...
    def large_scale_inference(self, cur_data, task):
        ...
        with torch.no_grad():
            ...
            for cur_candidate, score in zip(cur_candidates, result):
                ...
                if task == "quality" and cur_query_score >= 1.5:
                    cur_candidate.append(cur_query_score)
                    keep_candidate.append(cur_candidate)
                elif task == "ctr" and cur_query_score >= 0.05:
                    cur_candidate.append(cur_query_score)
                    keep_candidate.append(cur_candidate)
                else:
...
def dump_data_for_dpo(cur_data):
    search_query = "\n".join(cur_data["search_query"]).strip()
    clicked_web_title = "\n".join(cur_data["clicked_web_title"]).strip()
    clicked_news_title = "\n".join(cur_data["clicked_news_title"]).strip()
    edge_clicks = "\n".join(cur_data["edge_clicks"]).strip()
    chosen = "\n".join(cur_data["chosen"]).strip()
    rejected = "\n".join(cur_data["rejected"]).strip()

    prompt = {"number": 10, "search_query": search_query,
              "clicked_web_title": clicked_web_title,
              "clicked_news_title": clicked_news_title,
              "edge_clicks": edge_clicks}

    prompt = distillation_prompt_template.format(**prompt)
    return json.dumps({"prompt": prompt, "chosen": chosen, "rejected": rejected}, ensure_ascii=False)
    ...
    with open(args.output_path, "w") as f_write:
        for idx, cur_data in tqdm(enumerate(all_data)):
            try:
                ...
                cur_data = dump_data_for_dpo(cur_data)

                f_write.write(cur_data + "\n")
            except:
                continue
