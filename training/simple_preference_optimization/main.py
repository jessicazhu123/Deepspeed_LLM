...
def forward_calculation(args, policy_model, batch, device):
    ...
    # calculate the avg log p
    all_logps = (per_token_logps * batch["mask_loss"][:, 1:]).sum(-1) / (batch["mask_loss"][:, 1:]).sum(-1)
    policy_chosen_logps = all_logps[:pair_num]
    policy_rejected_logps = all_logps[pair_num:]

    pi_logratios = policy_chosen_logps - policy_rejected_logps
    gamma_logratios = args.gamma / args.beta
    logits = pi_logratios - gamma_logratios

    loss = -torch.nn.functional.logsigmoid(args.beta * logits).mean()
    chosen_rewards = args.beta * policy_chosen_logps.detach()
    rejected_rewards = args.beta * policy_rejected_logps.detach()
...
def main():
    ...
    def evaluation_reward(args, policy_model, device, eval_dataloader):
        policy_model.eval()
        ...
        for _step, _batch in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                loss, chosen_rewards, rejected_rewards = forward_calculation(args, policy_model, _batch, device)
    ...
    print_rank_0(
        ...
        args.global_rank)
    reward_score, reject_score, acc = evaluation_reward(args, policy_model, device, eval_dataloader)
    print_rank_0(
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            loss, chosen_rewards, rejected_rewards = forward_calculation(args, policy_model, batch, device)
            ...
            if args.eval_interval and (total_micro_steps % args.eval_interval == 0):
                ...
                reward_score, reject_score, acc = evaluation_reward(args, policy_model, device,
                                                                    eval_dataloader)
                print_rank_0(
